...
Page({
  ...
  onLoad: function (options) {
    ...
    let isOn = swan.getStorageSync('turnonWifiPrompt') == 0 ? true : false; //检查非wifi开启状态
    console.log(isOn);
    if (isOn === "") {
      //如果没有本地缓存说明是第一次载入，默认是开启状态
      isOn = true;
    }
    console.log(isOn);
    self.setData({
  ...
  clearStorage: function () {
    ...
    let storages = swan.getStorageInfoSync().keys;
    ...
    for (let i = 0; i < storages.length; i++) {
      ...
      console.log(storage);
      swan.removeStorageSync(storage);
    }

    let storages1 = swan.getStorageInfoSync();
  ...
  switchChange: function (e) {
    ...
    console.log(isOn);
    swan.setStorageSync('turnonWifiPrompt', isOn);
  }
});