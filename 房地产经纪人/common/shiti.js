...
function setModelRealMarkAnswerItems(jie_answer_array, nums, isModelReal, isSubmit, self) {
  ...
  for (let i = 0; i < jie_answer_array.length; i++) {
    ...
    if (select != "材料题") {
      if (isModelReal && isSubmit == false) {
        //如果是真题或者押题并且没有提交
        if (jie_answer_array[i].done_daan != "") {
          //如果答案不为空
      ...
      } else if (jie_answer_array[i].isRight == 0) {
        //如果题是正确的
        style = "background:#90dd35;color:white;border:1rpx solid #90dd35;";
      } else if (jie_answer_array[i].isRight == 1) {
        //如果题是错误的
      ...
      for (let j = 0; j < done_daan.length; j++) {
        ...
        } else if (daan.isRight == 0) {
          //如果题是正确的
          style = "background:#90dd35;color:white;border:1rpx solid #90dd35;";
        } else if (daan.isRight == 1) {
          //如果题是错误的
          style = "background:#fa4b5c;color:white;border:1rpx solid #fa4b5c;";
        }
...
function setMarkAnswer(shiti, isModelReal, isSubmit, self) {
  ...
  if (isModelReal && isSubmit == false) {
    //如果是真题或者押题
    style = "background:#0197f6;color:white;border:1rpx solid #0197f6; ";
  } else if (shiti.flag == 0) {
    //如果题是正确的
    style = "background:#90dd35;color:white;border:1rpx solid #90dd35; ";
  } else if (shiti.flag == 1) {
    //如果题是错误的
    style = "background:#fa4b5c;color:white;border:1rpx solid #fa4b5c; ";
  }
...
function storeAnswerStatus(shiti, self) {
  ...
  let answer_nums_array = swan.getStorageSync("shiti" + self.data.zhangjie_id + username);
...
function storeModelRealAnswerStatus(shiti, self) {
  ...
  let answer_nums_array = swan.getStorageSync(self.data.tiTypeStr + "modelReal" + id + username)?swan.getStorageSync(self.data.tiTypeStr + "modelReal" + id + username):[];
  ...
  for (let i = 0; i < answer_nums_array.length; i++) {
    ...
    if (done_shiti_storage.id == shiti.id) {
      //已经存储过
...
function changeSelectStatus(done_daan, shiti, ifSubmit) {
  ...
  switch (shiti.tx) {
    ...
    case "多选题":
      ...
      for (let i = 0; i < done_daan.length; i++) {
        if (answers.indexOf(done_daan[i]) >= 0) {
          //如果正确答案包含选中
...
function ifDoneAll(shitiArray, doneAnswerArray) {
  if (shitiArray.length == doneAnswerArray.length) {
    //所有题都答完了
    swan.showToast({
      title: '所有题已经作答',
      duration: 4000
...
function lianxiRestart(self) {
  ...
  swan.setStorageSync("shiti" + self.data.zhangjie_id + username, answer_nums_array); //重置已答数组
...
function restartModelReal(self) {
  ...
  let answer_nums_array = swan.getStorageSync(self.data.tiTypeStr + "modelReal" + self.data.id + username); //将已答答案置空
  swan.setStorage({
