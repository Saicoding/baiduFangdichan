...
Page({
  ...
  onLoad: function (options) {
    app.setPageInfo();
    swan.setNavigationBarTitle({ //设置标题
      title: options.title
    });
    //获取是否有登录权限
    let self = this;
  ...
  onReady: function () {
    ...
    self.myVideo = swan.createVideoContext('myVideo');
  ...
  onShow: function () {
    ...
    let windowWidth = swan.getSystemInfoSync().windowWidth;
    ...
    self.videoContext = swan.createVideoContext('myVideo');

    share.monitorConnectType(self);

    let lastpx = swan.getStorageSync('lastVideo' + kcid + user.username);
    ...
    if (myproduct == buied) {
      ...
      app.post(API_URL, "action=getCourseShow&LoginRandom=" + LoginRandom + "&zcode=" + zcode + "&kcid=" + kcid, false, false, "", "").then(res => {
        ...
        for (let i = 0; i < videos.length; i++) {
          let cv = swan.createCanvasContext('myCanvas' + i, self);
          my_canvas.push(cv);
        }
  ...
  changeVideo: function (e) {
    ...
    let playTime = 0;
    if (currentTime > 10 && currentTime < lastVideo.length - 10) {
      //播放时间)
  ...
  tooglePlay: function () {
    ...
    if (currentVideo.lastViewLength == "0") {
      //如果没有播放过,就
  ...
  initVideos: function (videos, px, my_canvas) {
    for (let i = 0; i < videos.length; i++) {
      ...
      //初始化已观看视频的时间数组
      if (video.playCourseArr.length == 0) {
        //如果是空数组，说明是首次观看
        if (s !== "00") {
          for (let i = 0; i < m + 1; i++) {
  ...
  onHide: function () {
    ...
    if (user != undefined) {
      let kcid = self.data.kcid;
      let px = self.data.px;
      swan.setStorageSync('lastVideo' + kcid + user.username, px);
    }
  },
  ...
  onUnload: function () {
    ...
    swan.setStorageSync('lastVideo' + kcid + user.username, px);
    swan.setStorageSync('page', myproduct);
