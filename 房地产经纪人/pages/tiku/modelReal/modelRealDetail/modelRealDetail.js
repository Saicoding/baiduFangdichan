...
const winHeight = swan.getSystemInfoSync().windowHeight;
const app = getApp();
...
Page({
  ...
  onLoad: function (options) {
    ...
    let last_model_real = swan.getStorageSync(last_view_key); //得到最后一次的题目
    ...
    app.post(API_URL, "action=SelectTestShow&sjid=" + id + "&username=" + username + "&acode=" + acode, false, true, "", "", true, self).then(res => {
      ...
      let isSubmit = swan.getStorageSync(tiTypeStr + 'modelRealIsSubmit' + options.id + username);

      //开始计时
      let interval = "";
      if (!isSubmit) {
        //如果没提交
        let second = swan.getStorageSync(tiTypeStr + 'last_time' + options.id + username);
        if (second) {
        ...
        //如果已提交
        let last_gone_time_str = swan.getStorageSync(tiTypeStr + "last_gone_time" + options.id + username);
  ...
  sliderChange: function (e) {
    ...
    let lastStr = "#q" + px;
    console.log(shitiArray[px-1].px)
  ...
  onShow: function (e) {
    ...
    new Promise((resolve, reject) => {
      ...
      if (sliderShitiArray[res].TX == 99) {
        //如果已经提交试卷，才去判断题是否超过屏幕
  ...
  _submit: function () {
        ...
        case "多选题":
          if (doneAnswer.isRight == 0) {
            ...
            for (let m = 0; m < doneAnswer.done_daan.length; m++) {
              ...
              if (shitiArray[px - 1].answer.indexOf(done) == -1) {
                //如果不包含
                has = false;
              } else {
                num++;
            ...
            if (daan.isRight == 0) {
              ...
              if (typeof daan.done_daan == "object") {
                ...
                for (let m = 0; m < daan.done_daan.length; m++) {
                  ...
                  if (shitiArray[px - 1].xiaoti[daan.px - daan.clpx - 1].answer.indexOf(done) == -1) {
                    //如果不包含
                    has = false;
                  } else {
                    num++;
    ...
    //提交结果
    app.post(API_URL, "action=SaveTestResult" + "&username=" + username + "&acode=" + acode + "&sjid=" + id + "&userAnswer1=" + doneUserAnswer.userAnswer1 + "&userAnswer2=" + doneUserAnswer.userAnswer2 + "&userAnswer99=" + doneUserAnswer.userAnswer99 + "&tid1=" + doneUserAnswer.tid1 + "&tid2=" + doneUserAnswer.tid2 + "&tid99=" + doneUserAnswer.tid99 + "&rightAnswer1=" + doneUserAnswer.rightAnswer1 + "&rightAnswer2=" + doneUserAnswer.rightAnswer2 + "&rightAnswer99=" + doneUserAnswer.rightAnswer99 + "&testTime=" + gone_time + "&testScore=" + score + "&TrueTid=" + doneUserAnswer.TrueTid, true, true, "计算中").then(res => {

      if (score > self.data.test_score) {
        //如果比历史分数高就更新
        let pages = getCurrentPages();
        let prevPage = pages[pages.length - 2]; //上一个页面
