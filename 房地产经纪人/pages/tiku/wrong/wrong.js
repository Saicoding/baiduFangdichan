  ...
  */
  ...
  sliderChange: function (e) {
    ...
    if (direction == "左滑") {
      px++;
      if (px % 10 >= 6) {
        //滑动到号大于7，这时判断有没有下一个page
        let nextPage = (px - 1 - (px - 1) % 10) / 10 + 2;

        if (pageArray.indexOf(nextPage) == -1 && nextPage <= pageall) {
          //已渲染数组不包含下一页面
      ...
      if (px % 10 <= 3) {
        //滑动到小于等于3时，这时判断有没有上一个page
        let prePage = (px - 1 - (px - 1) % 10) / 10;

        if (pageArray.indexOf(prePage) == -1 && prePage >= 1) {
          //已渲染数组不包含下一页面
  ...
  _tapEvent: function (e) {
    ...
    if (pageArray.indexOf(page) == -1) {
      ...
      if (px % 10 >= 1 && px % 10 <= 4 && pageArray.indexOf(prepage) == -1) {
        //如果是页码的第一题,并且有上一页,并且不在已渲染数组中
